...
# # # # # dir = 'ax201/2n_noR_iperf_0'
# # # # dir = 'ax201/2n_siR_siI_iperf_0'
...
# # # # # dir = 'awus/2n_noR_iperf_0'
# # # # dir = 'awus/2n_siR_siI_iperf_0'
# # # # main_title = f'{card}\n\nNodos y RSSI a lo largo del tiempo'
# # # # # dir = 'awus/2n_siR_siI_iperf_0'
# # # # # main_title = f'{card}. Scenario 4'

card = 'Alfa2 (Alfa AWUS036AXML)'
dir_memoria = 'awusAXML-img/memoria/'
dir = 'awusAXML/2n_noR_iperf_0'
# dir = 'awusAXML/2n_siR_siI_iperf_0'
main_title = f'{card}\n\nNodos y RSSI a lo largo del tiempo'
# dir = 'awusAXML/2n_siR_siI_iperf_0'
# main_title = f'{card}. Scenario 4'
...
# title1 = f'RSSI and nodes over time'
title1 = f''
title2 = f'Tasa de bits a lo largo del tiempo'
...
axis[0].plot(time_series, rssi_values, color='blue', label='RSSI (dBm)', zorder=1)
axis[0].set_xlabel('Tiempo (s)')
axis[0].set_ylabel('RSSI (dBm)', color='blue')
axis[0].tick_params(axis='y', labelcolor='blue')
...
# ax2 = axis[0].twinx()
axis[1].plot(time_series, bitrate_values, color='red', label='Bitrate (Mbps)')
axis[1].set_xlabel('Tiempo (s)')
axis[1].set_ylabel('Bitrate (Mbps)', color='red')
axis[1].tick_params(axis='y', labelcolor='red')

# show min rssi
axis[0].scatter(rssi_min_i+1, rssi_min, c='red', marker='o', s=100, zorder=2)
axis[0].annotate(f'{rssi_min} dBm', 
                        xy=(rssi_min_i+1, rssi_min), 
                        xytext=(rssi_min_i+1 - 1.4, rssi_min - 4),
                        textcoords='data',
                        fontsize=12,
                        color='red')

# plt.gca().set_ylim([ min(rssi_values_no_none) - 10, max(rssi_values_no_none) + 10])
axis[0].set_ylim([ min(rssi_values_no_none) - 10, max(rssi_values_no_none) + 10])
...
for i in range(len(transform_nodes)):
    if i == 0 or transform_nodes[i] != transform_nodes[i - 1]:
        if dict[transform_nodes[i]] == 3:
            continue
        axis[0].annotate(f'nodo {dict[transform_nodes[i]]}', 
        ...
        axis[0].axvline(x=time_series[i], color='black', linestyle='--', alpha=0.5)
...
# axis[0].legend(lines + lines2, labels + labels2, loc='best')

# Customize the plots
# axis[0].set_title(title1)
axis[0].set_title(main_title, fontweight='bold')
axis[0].grid(True)
axis[1].set_title(title2, fontweight='bold')
axis[1].grid(True)
# plt.suptitle(main_title, fontweight='bold')
plt.subplots_adjust(hspace=0.26)
plt.savefig(f'{dir_memoria}plot.png', format='png')
plt.show()